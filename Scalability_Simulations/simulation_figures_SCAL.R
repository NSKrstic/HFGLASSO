...
if(Sim_Scal_Type=="SS"){
  ...
  Long_Simulation_Results_DF_FINAL = Long_Simulation_Results_DF %>% group_by(Sample_Size, `Measure Type`, Measure) %>% summarize(`Mean Measure`= mean(`Measure Score`), `SE Measure`=sd(`Measure Score`)/n())
  ...
  ### Take the means and standard errors:
  Long_Simulation_Results_DF_FINAL = Long_Simulation_Results_DF %>% group_by(Cardinality, `Measure Type`, Measure) %>% summarize(`Mean Measure`= mean(`Measure Score`), `SE Measure`=sd(`Measure Score`)/n())
