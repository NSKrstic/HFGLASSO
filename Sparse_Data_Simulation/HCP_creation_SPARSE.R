...
for(i in 1:Categ_Num){
  ...
  if(i == Categ_Num){
    ...
    Probs = runif(Categ_Count-5, min=0.1, max=0.9)
    
    #Standardize probabilities
    Probs = Probs/sum(Probs)
    
    #Create the design matrix
    Temp_DesignMatrix = t(rmultinom(Samp_Size, 1, prob=Probs))[,1:(Categ_Count-5)]
    Temp_Valid_DesignMatrix = t(rmultinom(Valid_Samp_Size, 1, prob=Probs))[,1:(Categ_Count-5)]
  }
  
}

#### ADJUSTMENT FOR THIS SIMULATION: MAKE 5 OF THE CATEGORIES SPARSE IN THE DESIGN MATRICES

#Randomly select which columns will NOT be sparse, then populate them using the above
Non_Sparse_Cols = sort(sample(1:Categ_Counts[4], 15))
Sparse_Cols = which(!1:20 %in% Non_Sparse_Cols)

DesignMatrix = matrix(0, nrow=nrow(Temp_DesignMatrix), ncol=Categ_Counts[4])
DesignMatrix[,Non_Sparse_Cols] = Temp_DesignMatrix

Valid_DesignMatrix = matrix(0, nrow=nrow(Temp_Valid_DesignMatrix), ncol=Categ_Counts[4])
Valid_DesignMatrix[,Non_Sparse_Cols] = Temp_Valid_DesignMatrix

#Delete the final 25 (10) observations in the design matrix (validation/test design matrix)
DesignMatrix = DesignMatrix[1:(nrow(DesignMatrix)-25),]
Valid_DesignMatrix = Valid_DesignMatrix[1:(nrow(Valid_DesignMatrix)-10),]

#Replace the final 25 (10) observations with those corresponding to the 5 sparse categories (5 each for training, 2 each for validation)
Replace_DesMat_Train = as.matrix(model.matrix(~factor(rep(1:5, each=5))-1))
Replace_DesMat_Valid = as.matrix(model.matrix(~factor(rep(1:5, each=2))-1))

Sparse_DesMat_Train = matrix(0, nrow=25, ncol=Categ_Counts[4])
Sparse_DesMat_Train[,Sparse_Cols] = Replace_DesMat_Train

Sparse_DesMat_Valid = matrix(0, nrow=10, ncol=Categ_Counts[4])
Sparse_DesMat_Valid[,Sparse_Cols] = Replace_DesMat_Valid

#Finalize the Design Matrices
DesignMatrix = rbind(DesignMatrix, Sparse_DesMat_Train)
Valid_DesignMatrix = rbind(Valid_DesignMatrix, Sparse_DesMat_Valid)
